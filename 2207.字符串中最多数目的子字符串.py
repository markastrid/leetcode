#
# @lc app=leetcode.cn id=2207 lang=python3
#
# [2207] 字符串中最多数目的子字符串
#
# https://leetcode.cn/problems/maximize-number-of-subsequences-in-a-string/description/
#
# algorithms
# Medium (30.80%)
# Likes:    47
# Dislikes: 0
# Total Accepted:    17.5K
# Total Submissions: 44.7K
# Testcase Example:  '"abdcdbc"\n"ac"'
#
# 给你一个下标从 0 开始的字符串 text 和另一个下标从 0 开始且长度为 2 的字符串 pattern ，两者都只包含小写英文字母。
#
# 你可以在 text 中任意位置插入 一个 字符，这个插入的字符必须是 pattern[0] 或者 pattern[1] 。注意，这个字符可以插入在
# text 开头或者结尾的位置。
#
# 请你返回插入一个字符后，text 中最多包含多少个等于 pattern 的 子序列 。
#
# 子序列 指的是将一个字符串删除若干个字符后（也可以不删除），剩余字符保持原本顺序得到的字符串。
#
#
#
# 示例 1：
#
#
# 输入：text = "abdcdbc", pattern = "ac"
# 输出：4
# 解释：
# 如果我们在 text[1] 和 text[2] 之间添加 pattern[0] = 'a' ，那么我们得到 "abadcdbc" 。那么 "ac"
# 作为子序列出现 4 次。
# 其他得到 4 个 "ac" 子序列的方案还有 "aabdcdbc" 和 "abdacdbc" 。
# 但是，"abdcadbc" ，"abdccdbc" 和 "abdcdbcc" 这些字符串虽然是可行的插入方案，但是只出现了 3 次 "ac"
# 子序列，所以不是最优解。
# 可以证明插入一个字符后，无法得到超过 4 个 "ac" 子序列。
#
#
# 示例 2：
#
#
# 输入：text = "aabb", pattern = "ab"
# 输出：6
# 解释：
# 可以得到 6 个 "ab" 子序列的部分方案为 "aaabb" ，"aaabb" 和 "aabbb" 。
#
#
#
#
# 提示：
#
#
# 1 <= text.length <= 10^5
# pattern.length == 2
# text 和 pattern 都只包含小写英文字母。
#
#
#

# @lc code=start
class Solution:
    def maximumSubsequenceCount(self, text: str, pattern: str) -> int:
        # 整理一下思路:我需要一个循环，分别是插入两个字符中的一个，然后再在字符中寻找，这里注意，如果插入pattern[0]处只需要往后找，而插入pattern[1]处则只需要往前去找
        text_list = list(text)
        count0 = 0
        count1 = 0
        # 也就是说，把pattern[0]加在头部和把pattern[1]加在尾部产生的结果相加即可
        # 如果是pattern[0]我们就先找到所有的pattern[0]个数，遇到pattern[0]就--，遇到pattern[1]就加上当前的个数
        num_p0 = 1
        num_p1 = 1
        for i in text:
            if i == pattern[0]:
                num_p0 += 1
            if i == pattern[1]:
                num_p1 += 1
        # print(num_p0)
        # print(num_p1)
        # 这里处理相同的情况
        if pattern[0] == pattern[1]:
            sum = 0
            for i in range(num_p0):
                sum += i
            return sum
        # 这里处理不同的情况
        re_text = reversed(text)
        for i in re_text:
            if i == pattern[0]:
                num_p0 -= 1
            if i == pattern[1]:
                count0 += num_p0
        for i in text:
            if i == pattern[1]:
                num_p1 -= 1
            if i == pattern[0]:
                count1 += num_p1
        return max(count1, count0)
        # @lc code=end
